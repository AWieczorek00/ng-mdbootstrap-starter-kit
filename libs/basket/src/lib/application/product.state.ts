...
import {
  PRODUCT_CONTEXT_PORT,
  ProductContextPort,
} from './ports/secondary/context/product.context-port';
    ...
    RemoveProductCommandPort
{
  constructor(
    ...
    @Inject(PRODUCT_CONTEXT_PORT)
    private _productContextPort: ProductContextPort,
    @Inject(PRODUCT_EVENT_PORT)
    private _productEventPort: ProductEventPort
  ) {}
  ...
  loadAllProducts(): Observable<void> {
    return this._getsAllProductsDtoPort
      ...
      .pipe(
        switchMap((products) =>
          this._productContextPort
            .setState({ all: products })
            .pipe(
              tap((data) =>
                this._productEventPort.productsChangedPrice({
                  newPrice: products.reduce(
                    (a, c) => a + c.price.value / 100,
                    0
                  ),
                })
              )
  ...
  removeProduct(command: RemoveProductCommand): Observable<void> {
    return this._productContextPort.select().pipe(
      ...
      map((oldContext) => ({
        ...oldContext,
        all: oldContext.all.filter((p) => p.name !== command.productName),
      })),
      switchMap((newContext) =>
        this._productContextPort
          ...
          .pipe(
            tap((data) =>
              this._productEventPort.productsChangedPrice({
                newPrice: newContext.all.reduce(
                  (a, c) => a + c.price.value / 100,
                  0
                ),
              })
